...
| D63 | Batalha/Afinidade | **Sistema de vantagens e fraquezas elementais implementado** — tabela 8×8 em `src/systems/battle/affinityChart.ts`. Multiplica dano de E01 (físico), E02 (elemental) e E08 (drain) pela afinidade do atacante vs. defensor. Multiplicadores: ×2.0 (vantagem clara, unidirecional), ×1.5 (par volátil, simétrico), ×0.5 (desvantagem, unidirecional). Pares caos (luz↔sombra, éter↔vazio) e pares voláteis (água↔sombra, éter↔sombra, luz↔vazio, sombra↔vazio) são simétricos. Ciclo clássico (água→fogo→vento→terra→água) é unidirecional. Label de efetividade ("Super Efetivo!", "Fraco...") embutido no `BattleEvent.label`. 22 testes em `affinityChart.test.ts`. | Dano físico (E01) afetado por afinidade porque o herói inteiro é elemental por natureza — até socos de um herói de Fogo são imbued com fogo. Isso torna as vantagens de tipo relevantes para todos os estilos de jogo, não só para builds com E02. |
| D64 | Batalha/Ressonância | **Sistema de Ressonância Crítica implementado** — ultimate em dupla em `src/systems/battle/resonance.ts`. 12 pares sinérgicos (4 DANO, 4 SUPORTE, 4 CURA), todos simétricos e sem sobreposição de pares entre tipos. Pré-requisitos: ambos ativos, ≥ Raro, ≥ 3★ vínculo (`bond ≥ 50`), ultimate 100%, não em Exausta. Variantes (Bruta/Elemental/Ágil) determinadas pelo atributo dominante médio do par. Forma Exaltada (×1.5) ativada se qualquer herói tem mutação. Estado Exausta pós-ativação: não pode agir por 1 turno, ultimate bloqueada por 2 turnos (`resonanceExaustaRemaining`). Sem limite de pares no time — qualquer combinação de 6 heróis é válida se todos forem sinérgicos. `BattleAction.partnerId` identifica o segundo herói; novos EventTypes `resonance_used` e `resonance_exausta`; ActionType `resonance` adicionado. 43 testes em `resonance.test.ts`. | Sem limite de pares no time por decisão do usuário: "se o jogador conseguir criar um time de 6 heróis em que todos eles tem ressonancia entre si, tudo bem". O único gate é que ambos do par específico estejam no campo ativo na hora da ativação. |
